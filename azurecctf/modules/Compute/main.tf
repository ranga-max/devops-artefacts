...
// subnet level nsg should definitely be created and to be tested
//resource "azurerm_subnet_network_security_group_association" "my_subnetnsg_association" {
  //count               = var.vm-count
  //subnet_id                 = data.azurerm_subnet.subnet[element(local.subnets,count.index % length(local.subnets))].id
  //subnet_id                 = azurerm_network_interface.nic[count.index % length(local.subnets)].subnet_id
  //network_security_group_id = var.existing_network_security_group_id == null ? azurerm_network_security_group.nsg.0.id : var.existing_network_security_group_id
//}
...
#To get the tls certificate use the below command
#terraform show -json | \
#jq -r '.values.root_module.child_modules[].resources[].values | select(.private_key_pem).private_key_openssh' > azurepoc.pem

#terraform show -json | \
#jq -r '.values.root_module.child_modules[].resources[].values | select(.private_key_pem).private_key_pem' > azurepoc.pem 
