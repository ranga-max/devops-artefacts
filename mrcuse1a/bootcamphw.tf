...
resource "aws_instance" "zookeepers" {
  ...
  associate_public_ip_address = var.publicip_flag
}

resource "aws_route53_record" "zookeepers" {
  count = var.zk-dns-count
...
resource "aws_route53_record" "connect-cluster" {
  ...
  records = var.share_mode == true ? ["${element(aws_instance.brokers.*.private_ip, count.index)}"] :  ["${element(aws_instance.connect-cluster.*.private_ip, count.index)}"]
}
...
resource "aws_route53_record" "schema" {
  ...
  records = var.share_mode == true ? ["${element(aws_instance.brokers.*.private_ip, count.index)}"] : ["${element(aws_instance.schema.*.private_ip, count.index)}"]
}
